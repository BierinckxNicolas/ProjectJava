	...
	function getHtml() {
		...
		tinymce.each(emoticons, function(row) {
			...
			tinymce.each(row, function(icon) {
				var emoticonUrl = url + '/images/smiley-' + icon + '.gif';

				emoticonsHtml += '<td><a href="#" data-mce-url="' + emoticonUrl + '" data-mce-alt="' + icon + '" tabindex="-1" ' +
					'role="option" aria-label="' + icon + '"><images src="' +
			...
			onclick: function(e) {
				...
				if (linkElm) {
					editor.insertContent(
						'<images src="' + linkElm.getAttribute('data-mce-url') + '" alt="' + linkElm.getAttribute('data-mce-alt') + '" />'
					);
