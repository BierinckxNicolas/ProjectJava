...
tinymce.PluginManager.add('image', function(editor) {
	...
	function showDialog(imageList) {
		...
		function onSubmitForm() {
			...
			editor.undoManager.transact(function() {
				...
				if (!imgElm) {
					data.id = '__mcenew';
					editor.focus();
					editor.selection.setContent(dom.createHTML('images', data));
		...
		if (figureElm) {
			imgElm = dom.select('images', figureElm)[0];
		}
					...
					function(item) {
						if (item.value) {
							item.textStyle = function() {
								return editor.formatter.getCssText({inline: 'images', classes: [item.value]});
							};
						}
					}
				)
